...
- フォーム送信 → `onFormSubmit(e)` → 注文一覧へ記録（必要に応じて顧客名簿更新）
...
- Googleスプレッドシート（テンプレ複製での導入推奨）
- Googleフォーム（質問タイトルは `Config.gs` の定義と一致させる）
- LINE公式アカウント（Messaging API のチャネルアクセストークンが必要）
...
- `Main.gs`：`onFormSubmit(e)` の入口
- `FormService.gs`：フォーム回答の解釈
- `OrderService.gs`：注文一覧への保存・集計
- `LineService.gs` / `LineWebhook.gs`：LINE送受信（Webhook受信は Webアプリ）
- `SetupTools.gs`：導入ツール（トリガー設定・日次準備設定など）
- `BackupService.gs`：自動/手動バックアップ
- `MenuVisibility.gs`：メニュー表示制御（Script PropertiesでON/OFF）
- `CustomerService.gs` / `CustomerForm.html`：顧客備考のサイドバー編集
- `Config.gs`：シート名/列番号/プロパティキー等の定義
...
### 0) テンプレ複製（推奨）
1. 既存のテンプレスプレッドシートを「ファイル → コピーを作成」で複製
2. 必要に応じてフォーム側の質問文を調整（`Config.gs` の `FORM` 定義に合わせる）

### 1) Script Properties を設定（必須/推奨）
...
（任意）自動返信表示名を使いたい場合は `AUTO_REPLY_NAME` 列を使ってください。
