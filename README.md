According to a document from **2026-02-08**（`repomix-output-tree-3.1.3.md`）を反映した、**現行版（v3.1.3相当）の GitHub 用 README.md**です。コピペして `README.md` として使ってください。
（※現行コードでは `氏名不一致ログ` や「理由テンプレ」「テンプレ用プロパティ」などが増えているので、そのぶん追記してあります。根拠は末尾に付けています）
